...
import { getOrdersApi } from "../../auth/auth.payment";
import type { Order } from "../../auth/auth.types";
        ...
        const fetchOrders = async () => {
            try {
                ...
                if (validData.length > 0) {
                    setSelectedOrderId(validData[0].id);
                }
            } catch (error) {
    ...
    if (isLoading) {
        return (
            <div className="flex h-screen items-center justify-center bg-gray-50">
                ...
                <span className="ml-3 font-medium text-gray-600">Loading orders...</span>
            </div>
        );
    }
    ...
    if (!orders || orders.length === 0 || !selectedOrder) {
        return (
            <div className="flex h-screen items-center justify-center bg-gray-50 flex-col">
                ...
                <p className="text-gray-500 mt-2">There are no orders available right now.</p>
            </div>
        );
    }
                    ...
                    {orders.map(order => (
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center mb-2">
                                ...
                                {/* FIX: Wrapped order.total in Number() */}
                                <span className="font-bold text-sm text-gray-900">${Number(order.total || 0).toFixed(2)}</span>
                            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
                ...
                <div className="bg-white border-b border-gray-200 px-8 py-5 flex justify-between items-center shadow-sm">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Placed on {selectedOrder.timeline?.[0]?.date || 'Unknown Date'} via Online Store</p>
                    </div>
                        ...
                        <div className="col-span-2 space-y-6">
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                                    <h3 className="font-bold text-gray-900">Order Items ({selectedOrder.items?.length || 0})</h3>
                                        ...
                                        <div key={item.id} className="p-4 flex gap-4 hover:bg-gray-50 transition-colors">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex justify-between">
                                                    ...
                                                    {/* FIX: Wrapped item.price in Number() */}
                                ...
                                <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 space-y-2">
                                    <div className="flex justify-between text-sm text-gray-500">
                                        <span>Subtotal</span>
                                        {/* FIX: Wrapped selectedOrder.total in Number() */}
                                        <span>${(Number(selectedOrder.total) * 0.9).toFixed(2)}</span>
                                    </div>
                                    <div className="flex justify-between text-sm text-gray-500">
                                        <span>Shipping</span>
                                        {/* FIX: Wrapped selectedOrder.total in Number() */}
                                        <span>${(Number(selectedOrder.total) * 0.1).toFixed(2)}</span>
                                    </div>
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                <div className="p-6 border-b border-gray-100">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <div className="h-8 w-8 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center text-sm font-bold">
                                            {selectedOrder.customer?.name?.charAt(0) || <User className="h-4 w-4"/>}
                                        </div>
                                    </div>
                                ...
                                <div className="p-4 bg-gray-50 border-b border-gray-100 grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-gray-900">${Number(selectedOrder.customer?.ltv || 0).toLocaleString()}</p>
                                    </div>
                                    <div>
