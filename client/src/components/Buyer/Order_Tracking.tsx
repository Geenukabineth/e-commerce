            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">Placed on {order.placed_on} • Total: <span className="font-bold text-gray-900">${order.total_amount.toFixed(2)}</span></p>
                    </div>
                    <div className="flex gap-3">
                        <button className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 transition">
                            <MessageSquare className="h-4 w-4" /> Need Help?
                        </button>
                        <button 
                            onClick={handleDownloadInvoice}
                            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 shadow-md transition"
                        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                        <h2 className="text-sm font-bold text-gray-500 uppercase tracking-wider">Shipments ({order.shipments.length})</h2>
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-2">
                                        <Package className={`h-5 w-5 ${activeShipmentId === shipment.id ? 'text-indigo-600' : 'text-gray-400'}`} />
                                        <div>
                                            <p className="font-bold text-sm text-gray-900">{shipment.vendor_name}</p>
                                            <p className="text-xs text-gray-500">{shipment.carrier}</p>
                                        </div>
                                    </div>
                                    {shipment.status === 'delivered' ? (
                                        <span className="bg-green-100 text-green-700 p-1 rounded-full"><CheckCircle className="h-4 w-4" /></span>
                                    ) : (
                                        <span className="bg-blue-100 text-blue-700 p-1 rounded-full"><Truck className="h-4 w-4" /></span>
                                    )}
                                </div>
                                ...
                                <div className="flex gap-2 overflow-hidden mb-3">
                                    {shipment.items.map(item => (
                                        <div key={item.id} className="h-12 w-12 bg-gray-100 rounded-md border border-gray-200 flex items-center justify-center flex-shrink-0">
                                            {/* Img Placeholder */}
                                            <Box className="h-6 w-6 text-gray-400 opacity-50" />
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="flex justify-between items-center pt-3 border-t border-gray-50">
                                    <span className={`text-xs font-bold uppercase ${
                                        ...
                                        shipment.status === 'out_for_delivery' ? 'text-indigo-600' : 'text-gray-500'
                                    }`}>
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm mt-8">
                            <h3 className="font-bold text-gray-900 mb-4 text-sm">Shipping Details</h3>
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs font-bold text-gray-500 uppercase">Contact Info</p>
                                        <p className="text-sm text-gray-800 mt-1">+1 (555) 019-2834</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            ...
                            <div className="h-48 bg-indigo-50 relative flex items-center justify-center border-b border-gray-200">
                                <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/map-light.png')] opacity-30"></div>
                                <div className="bg-white/90 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg flex items-center gap-3 z-10 animate-in fade-in zoom-in">
                            ...
                            <div className="p-8">
                                <div className="flex justify-between items-start mb-8">
                                    <div>
                                        <h2 className="text-2xl font-bold text-gray-900 mb-1">{activeShipment.status === 'delivered' ? 'Delivered' : 'Arriving Soon'}</h2>
                                        <p className="text-gray-500">
                                            Est. Delivery: <span className="font-bold text-gray-900">{activeShipment.estimated_delivery}</span>
                                        </p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs text-gray-500 uppercase font-bold mb-1">Tracking Number</p>
                                        <div className="flex items-center gap-2 text-indigo-600 bg-indigo-50 px-3 py-1 rounded-lg">
                                            ...
                                            <button className="hover:text-indigo-800"><Copy className="h-3 w-3" /></button>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            return (
                                                <div key={step} className="flex flex-col items-center gap-2">
                                                    <div className={`h-8 w-8 rounded-full flex items-center justify-center border-4 z-10 transition-colors ${
                                                        ...
                                                        {isCompleted ? <CheckCircle className="h-4 w-4" /> : <span className="h-2.5 w-2.5 bg-gray-300 rounded-full"></span>}
                                                    </div>
                                                    <span className={`text-xs font-bold absolute top-10 w-24 text-center ${isCurrent ? 'text-gray-900' : 'text-gray-400'}`}>
                                                        {step}
                                                    </span>
                                                </div>
                                            );
                                ...
                                <div className="mt-12 space-y-6 border-l-2 border-gray-100 ml-4 pl-8 relative">
                                    {activeShipment.events.map((event, idx) => (
                                        <div key={idx} className="relative">
                                            <div className="absolute -left-[41px] top-1 h-5 w-5 bg-gray-100 rounded-full border-2 border-white ring-1 ring-gray-200 flex items-center justify-center">
                                                <div className={`h-2 w-2 rounded-full ${idx === 0 ? 'bg-indigo-600' : 'bg-gray-400'}`}></div>
                                            </div>
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                            <h3 className="font-bold text-gray-900 mb-4 text-sm">Package Contents</h3>
                            <div className="space-y-4">
                                {activeShipment.items.map(item => (
                                    <div key={item.id} className="flex items-center gap-4 py-2 border-b border-gray-50 last:border-0">
                                        <div className="h-16 w-16 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 border border-gray-200">
